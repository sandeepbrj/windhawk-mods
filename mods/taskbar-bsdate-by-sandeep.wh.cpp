// ==WindhawkMod==
// @id              taskbar-bsdate-by-sandeep
// @name            Taskbar BS Date
// @description     Adds BS Date to System Tray
// @version         1.0
// @author          sandeep
// @github          https://github.com/sandeepbrj
...
* `%date%` - the date as configured by the date format in settings.
  ...
  * `%date_tz<n>%` - the date with a custom time zone. `<n>` is the time zone
    number in the list of time zones configured in the mod settings (not Windows
    settings).
* `%nepdate%` - the Nepali date in yyyy-mm-dd format.
...
static const int kNepaliMonthDays[91][12] = {
...
void GetNepaliDate(int daysPassed, int& outYear, int& outMonth, int& outDate) {
    if (daysPassed < 1 || daysPassed > 33238) {
        outYear = 2000;
        outMonth = 0;
        outDate = 1;
        return;
    ...
    int yearIdx = 0;
    for (yearIdx = 0; yearIdx < 91; ++yearIdx) {
        int daysInYear = 0;
        for (int m = 0; m < 12; ++m) {
            daysInYear += kNepaliMonthDays[yearIdx][m];
        }
    ...
    outYear = 2000 + yearIdx;

    int monthIdx = 0;
    for (monthIdx = 0; monthIdx < 12; ++monthIdx) {
        int daysInMonth = kNepaliMonthDays[yearIdx][monthIdx];
    ...
    outMonth = monthIdx;
    outDate = remainingDays;
}

FormattedString<FORMATTED_BUFFER_SIZE> g_nepdateFormatted;

PCWSTR GetNepdateFormatted() {
    if (g_nepdateFormatted.formatIndex != g_formatIndex) {
        int year = g_formatTime.wYear;
        int month = g_formatTime.wMonth;
        int day = g_formatTime.wDay;

        int targetJdn = GetJulianDay(year, month, day);
        int epochJdn = GetJulianDay(1943, 4, 13);
        int daysPassed = targetJdn - epochJdn;

        int nepYear = 2000, nepMonth = 0, nepDay = 1;
        GetNepaliDate(daysPassed, nepYear, nepMonth, nepDay);

        int displayMonth = nepMonth + 1;
        swprintf_s(g_nepdateFormatted.buffer, ARRAYSIZE(g_nepdateFormatted.buffer),
                   L"%04d-%02d-%02d", nepYear, displayMonth, nepDay);

        g_nepdateFormatted.formatIndex = g_formatIndex;
    }
    return g_nepdateFormatted.buffer;
}
...
size_t ResolveFormatToken(
    ...
    struct {
        ...
        {L"%nepdate%"sv, GetNepdateFormatted},
